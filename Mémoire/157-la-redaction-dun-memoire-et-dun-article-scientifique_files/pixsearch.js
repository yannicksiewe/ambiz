...
*/
function setSpecifiedLanguage(a,b,c,d,e,f,g,h,i,j,k,l,m,n){
	_txtResults	= a;
	_txtClose	= b;
	_txtSearch	= c;
	_txtReadmore= d;
...
window.addEvent('domready', function() {
				...
				res_data.each(function(r) {
					...
					if(r.getTag() == "div"){
						if(r.getChildren().length > 2){
							...
							var link = new Element('a').setProperty('href',r.getFirst().getNext().getProperty('href')).injectInside(el);
							...
							var link = new Element('a').setProperty('href',r.getFirst().getNext().getProperty('href')).setHTML(_txtReadmore).injectAfter(br);
	...
	$('ps_search_str').onkeyup = function(){
		...
		var rq = req.request.delay(500,req);
